...
function FillApplicantDetails(code) {
    $('#AjaxLoader').show();
    $.ajax({
        url: '/JQuery/GetConDelForTankBooking',
        data: { AP_Code: code },
        type: 'GET',
        dataType: 'json',
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data != null && data != undefined && data.IsSuccess == true) {
                    ...
                    window.location.href = '/Transaction/TankbookingList';
                    //setTimeout(function () {
            ...
            success: function (data) {
                if (data != null && data != undefined && data.IsSuccess == true) {
                    if (data.Result == -1) {
                        ...
                        Refresh();
                        toastr.success(data.Message);
                        var win = window.open('/Report/TankBooking/' + data.Result + '', '_blank');
                        if (win) {
...
function fetchData() {
    ...
    window.location.href = url + "/Transaction/TankbookingList?fromdates=" + fromdates + "&todates=" + todates + "&AP_Code=" + AP_Code;
    return false;
}