        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data != null && data != undefined && data.IsSuccess == true) {
                    Calltoastrsuccessredirectanotherpage(data.Message, "/Transaction/ConsumerInvoiceList")
                }
                else {
...
function QtyOnkeyUp(IM_ItemCode) {
    ...
    //$('#GS_AdjAmt').val(AlocateAmt.toFixed(2));
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data != null && data != undefined && data.IsSuccess == true) {
                    ...
                    if (data.Result == 100) {
                        //Refresh();
                        window.location.href = '/Transaction/ConsumerInvoice';
                        //window.location.href = '/Transaction/ReConnectionList';
                        var win = window.open('/Report/ConsumerInvoice?GS_BillNo=' + $('#GS_BillNo').val() + '', '_blank');
                        if (win) {
...
function fetchData() {
    ...
    window.location.href = url + "/Transaction/ConsumerInvoiceList?fromdate=" + fromdate + "&todate=" + todate + "&GS_PartyCode=" + GS_PartyCode;
    return false;
}
